...
var app = {
	...
	onDeviceReady: function () {
		app.wikitudePlugin = cordova.require(
			"com.wikitude.phonegap.wikitudeplugin.WikitudePlugin"
		);
		...
		app.loadExampleARchitectWorld(getSamplePath(5, 0));
	},
	continueLoadingExampleARchitectWorld: function (example) {
		...
		if (example.path.length > cordova.file.applicationDirectory) {
			if (
				example.path.substring(0, cordova.file.applicationDirectory) !=
	...
	loadARchitectWorld: function (architectWorld) {
		app.wikitudePlugin.loadARchitectWorld(
			function successFn(loadedURL) {
				...
				if (
					architectWorld.requiredExtension ===
					"ObtainPoiDataFromApplicationModel"
				) {
	...
	showBuildInformation: function () {
		...
		app.wikitudePlugin.getSDKBuildInformation(function (buildInformationJSON) {
			...
			alert(
				"Build configuration: " +
					buildInformation.buildConfiguration +
					"\n" +
					"Build date: " +
					buildInformation.buildDate +
					"\n" +
					"Build number: " +
